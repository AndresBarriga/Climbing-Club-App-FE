...
export default function RecoverPasswordCreation() {
  ...
  const checkToken = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/create-password/check-recoverpassword-token`, {
        method: "GET",
        headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    fetch(`${process.env.REACT_APP_BACKEND_URL}/api/create-password/new-password`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}` 
 ...
 <div className="modal-content">
  <h2>Token has been already used or has expired. Please try restoring your password again.</h2>
